const TimelineItem2 = ({ timeline: { title, meta, text, year } }) => {
    return (
        <div className="timeline relative block items-start rounded-full border-white border-opacity-20 first:mt-0 md:flex">
            <span className="timeline-year absolute top-0 left-12 mt-7 mr-12 min-w-[250px] rounded-full bg-primary bg-opacity-10 p-2 text-center text-base leading-none md:relative md:left-0">
                {year}
            </span>
            <div className="timeline-right relative border-l-4 border-white border-opacity-20 pt-16 pl-12 md:pt-6">
                ...
                <h4 className="mb-2 text-heading">{meta}</h4>
               {/* Conditional rendering for text array */}
               {Array.isArray(text) ? (
                    <ul>
                        {text.map((item, index) => (
